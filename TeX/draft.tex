...
\KwOut{\textsc{False} if $j$ is not a sporadic $j$-invariant. \abbey{Update: Finite list of curves, degrees such that any isolated point on $X_1(n)$ associated to $E$ maps down under the natural projection map to a point in this list. If empty, $j$ is not isolated.)}}
...
Reduce level\;
For all $12 < b \mid N$, using Algorithm~\ref{alg:compute_degree} compute the minimum degree of the field of definition of a point with respect to $G \mod b$, the image of $G$ in $\GL_2(\Z/b\Z)$\;
\Return{?} \;
...
\begin{algorithm}[H]
  ...
  \KwOut{$G\subseteq \GL_2(\Z/m_0\Z)$ such that $G=\im\rho_{E,m_0}$ and $\im\rho_{E,m^{\infty}}=\pi^{-1}(\im\rho_{E,m_0})$ where $m$ is the product of the non-surjective primes and $2$ and $3$ (if they do not already divide $m$), and $m_0$ is minimal \abbey{Don't we need to include 2 and 3 in our definition of $m$? It is possible 2 and 3 may divide $m_0$ even if they are not surjective primes. If they are not necessary, then 2 and 3 need not divide $m_0$ in the final output.} } 
  \caption{Reduce level}%
  \label{alg:reduce_level}
  Let $N_E = \{\ell|N \text{ prime} \colon \rho_{E,\ell} \text{ is non-surjective} \}$\;
  Let $S_E= N_E \cup \{ 2, 3\}$\;
  Let $m_0 = \prod_{\ell \in S_E} \ell^{v_{\ell}(N)}$\;
  \For{$\ell \in \{ \ell | m_0 \colon \ell_0 \text{ prime} \}$}{
  \While{$\ell|m_0$ and $\#G/\#(G \pmod{m_0/p}) = p^4$}{
    $m_0 = m_0/p$\;
    $G = G \pmod{m_0}$\;
  }
  \If{not $p$ in $N_E$ and $\ord_p(m_0) = 1$ and $\#G/\#(G \pmod{m_0/p}) = \#\GL_2(\Z/p\Z)$}{
  $m_0 = m_0/p$\;
   $G = G \pmod{m_0}$\;}}
  \Return{$G,m_0$}
\end{algorithm}
...
\begin{algorithm}[H]\label{alg:compute_degree}
  \KwIn{$G\subseteq \GL_2(\Z/N\Z)$ such that $\im\rho_E=\pi^{-1}(G)$ and $N$ is the level; a bound $g \in \Z$}
  \KwOut{all degrees of $x$-coordinates of points that are $\geq g$} 
  Let $H \colonequals \langle G, -I_2\rangle \leq \GL_2(\Z/N\Z)$\;
  Compute the orbits of $H$ acting on $(\Z/N\Z)^2$\label{Inc-1}\;
  Filter the orbit representatives that have order equal to $N$\;
  Compute the orbit sizes ``degrees'' $s$ such that $s/2 \geq g$\label{DivBy2}\;
