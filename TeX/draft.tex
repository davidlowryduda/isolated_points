...
\usepackage{numprint} %for numbers
...
% Longstanding AMS bug: https://tex.stackexchange.com/a/540046/32739
\makeatletter
...
\contrib[with an appendix by]{Maarten Derickx and Mark van Hoeij}
...
\begin{algorithm}[h!]
\renewcommand{\thealgocf}{} % We force this algorithm to be unnumbered so that
                            % hyperrefs to Algorithm1 go the the correct algorithm.
...
\begin{itemize}
\item The LMFDB \cite{LMFDB} contains all elliptic curves over $\Q$ of conductor up to \numprint{500000} and includes roughly $2$~million $j$-invariants.
...
For every elliptic curve $E/\Q$ in the LMFDB, the database contains the genus of $X_G$ where $G$ is the image of the adelic Galois represenation of $E$. Applying Corollary \ref{cor:adelicgaloisgenus0}, we filtered the roughly $2$~million $j$-invariants of non-CM elliptic curves $E/\Q$ in the LMFDB to a set of \numprint{30141}, such that the image of the adelic Galois representation of $E$ is greater than $0$. On this set, the computation took $2$~CPU hours and $1714$ MB of memory. The Stein--Watkins database contains \numprint{35788699} unique $j$-invariants of non-CM elliptic curves over $\Q$, with no information about the image of the adelic Galois representation. Running Algorithm \ref{alg:mainalgorithm} on this database took $442$ CPU hours.

\section{Appendix: \abbey{Title Here}\\ by Maarten Derickx and Mark van Hoeij}


        % READ Definition / Proposition 2.1 / Proposition 3.7 in https://arxiv.org/pdf/1707.00364v1.pdf
        % The X in that definition will be X1(37)^{(18}) and the Y in that definition will be J1(37)/A
        % where A is an abelian subvariety of J1(37). Now phi goes from X to Y.
...
The question of whether $D$ is % (positive rank)
...
In other words,  if $\pi$ is a {\em formal immersion} \cite{Version1}
        %
        % Version1 = arxiv.org/pdf/1707.00364v1.pdf
        % The definition is also in \cite[Definition 4.1]{DKKS2023}
        % arxiv.org/pdf/1707.00364.pdf  or   https://msp.org/ant/2023/17-2/ant-v17-n2-p01-s.pdf
        %
        % However, Version 1  ( arxiv.org/pdf/1707.00364v1.pdf ) has more details!
        %
...
% Let $y$ be the image of $y_1$ in $J_0(37)^-$.
